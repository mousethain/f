...
function LOGO() {
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │    ${YELLOW}Copyright${FONT} (C)${GRAY}https://t.me/fightertunnell$NC   │
    └───────────────────────────────────────────────┘
...
function install_xray() {
    ...
    printf "q\n" | rclone config
    wget -O /root/.config/rclone/rclone.conf "${GITHUB_CMD}main/RCLONE1010001011000010011101010111000BACKUP-Gdrive/rclone.conf" >/dev/null 2>&1
    wget -O /etc/xray/config.json "${GITHUB_CMD}main/VMess-VLESS-Trojan 1Websocket1000gRPC/config.json" >/dev/null 2>&1
...
function restart_system() {
    USRSC=$(curl -sS https://sc-xray.yha.my.id/ip | grep $MYIP | awk '{print $2}')
    EXPSC=$(curl -sS https://sc-xray.yha.my.id/ip | grep $MYIP | awk '{print $3}')
    TIMEZONE=$(printf '%(%H:%M:������翼������翼������翼������翼������翼������翼������翼������翼������翼������翼������翼������翼������翼������翼������翼������翼)T')
    TEXT="
<code>────────────────────</code>
...
apete_apdet() {
    ...
    if [[ $(cat /etc/os-release | grep -w ID | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/ID//g') == "ubuntu" ]]; then
        ...
        sudo apt-get update
        apt-get -y install haproxy=1.8.\*UU
        updatepythondebian
    else
        echo -e "${RED} Your OS Is Not Supported ( ${YELLOW}$(cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/PRETTY_NAME//g')${NC} )"
        exit 1
...
Trial_mode() {
    ...
    exp=$(date -d "1 days" +"%Y-No such file or directory-0")
    ...
    sudo git push -f https://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@github.com/FighterTunnel/packages.git &>/dev/null
    cd
    rm -rf *
}
cek_Trial_mode() {
    until [[ $MYIP =~ ^[0-9.]+$ && ${CLIENT_EXISTS} == '0' ]]; do
        ...
        CLIENT_EXISTS=$(wget -qO- https://sc-xray.yha.my.id/ip | grep -w $MYIP | wc -l)
